...
short rc_fnd_file(rc_xdsid *xdsid)
{
	...
	dsid_list = (rc_dsid_list *)gtm_malloc_intern(sizeof(rc_dsid_list));
	...
	if (fpath2.len > MAX_FN_LEN)
	    return RC_BADFILESPEC;
	dsid_list->fname = (char *)gtm_malloc_intern(fpath2.len + 1);
	...
	*((char*)(dsid_list->fname + fpath2.len)) = 0;
	...
	if (cs_data->reserved_bytes < RC_RESERVED)
	{
		...
		fpath2.addr));
	    gtm_free_intern(dsid_list->fname);
	    dsid_list->fname=0;
	    gtm_free_intern(dsid_list);
	    dsid_list=0;
	...
	cs_addrs->dir_tree = (gv_namehead*)gtm_malloc_intern(sizeof(gv_namehead) + 2*sizeof(gv_key) + 3*(gv_keysize - 1));
	...
	g->root = DIR_ROOT;
	dsid_list->gda = (gd_addr*)gtm_malloc_intern(sizeof(gd_addr) + 3 * sizeof(gd_binding));
	...
	dsid_list->gda->tab_ptr = (htab_desc *)gtm_malloc_intern(sizeof(htab_desc));
	ht_init(dsid_list->gda->tab_ptr,0);
	change_reg();
	if (rc_overflow->top < cs_addrs->hdr->blk_size)
	{	if (rc_overflow->buff)
		    gtm_free_intern(rc_overflow->buff);
		    rc_overflow->top = cs_addrs->hdr->blk_size;
		    rc_overflow->buff = (char*)gtm_malloc_intern(rc_overflow->top);
	...
	fdi_ptr = (rc_dsid_list *)gtm_malloc_intern(sizeof(rc_dsid_list));
	fdi_ptr->fname = (char *)gtm_malloc_intern(len+1);
	fdi_ptr->dsid = dsid;
	...
	*(fdi_ptr->fname + (len)) = 0;
	...
	else if (cs_data->dsid != dsid || cs_data->rc_node != node)
	{
	    ...
	    OMI_DBG((omi_debug, "RC Rq DSID:  %d\tRC Node:  %d\n",
		     dsid,node));
	    gtm_free_intern(fdi_ptr->fname);
	    fdi_ptr->fname=0;
	    gtm_free_intern(fdi_ptr);
	    fdi_ptr=0;
	    gtm_free_intern(gv_cur_region->dyn.addr);
	    gv_cur_region->dyn.addr=0;
	    gtm_free_intern(gv_cur_region);
	...
	cs_addrs->dir_tree = (gv_namehead*)gtm_malloc_intern(sizeof(gv_namehead) + 2*sizeof(gv_key) + 3*(keysize - 1));
	...
	g->clue.prev = g->clue.end = 0;
	g->root = DIR_ROOT;
	fdi_ptr->gda = (gd_addr*)gtm_malloc_intern(sizeof(gd_addr) + 3 * sizeof(gd_binding));
	...
	map->reg.addr = gv_cur_region;
	memset(map->name,-1,MAX_NM_LEN);
	fdi_ptr->gda->tab_ptr = (htab_desc *)gtm_malloc_intern(sizeof(htab_desc));
